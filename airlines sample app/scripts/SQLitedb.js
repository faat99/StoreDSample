...
app.createTable = function() {
	...
	db.transaction(function(tx) {
		tx.executeSql("CREATE TABLE IF NOT EXISTS tblApp(ID INTEGER PRIMARY KEY ASC, AssetID INTEGER, ClientLocationID INTEGER, ContentsDescription TEXT, ClientLocation TEXT, Status TEXT, CurrentLocation TEXT, CurrentLocationID INTEGER, ContentDescription TEXT, AssetClientID INTEGER, TransportOrderAssetID INTEGER, TransportOrderID INTEGER, TargetLocation TEXT, TOAStatus TEXT, CompletionStamp DATETIME, ClientID INTEGER, CompletionTimeStamp DATETIME, TOStatus TEXT, DriverName TEXT, PlannedDate DATETIME, PlannedTimeStart DATETIME, PlannedTimeEnd DATETIME, Notes TEXT, Sequence INTEGER, DriverAppPW TEXT)", [], app.insertJSON, app.onError);
	});
}
...
app.logincheck = function(drivername){
    ...
    var db = app.db;
	db.transaction(function(tx) {
		tx.executeSql("SELECT DISTINCT DriverAppPW FROM tblApp WHERE DriverName=? ORDER by TransportOrderID DESC", [drivername], 
					  render, 
					  app.onError);
	});        
}
...
function check2(toaid,number,clientid){
    ...
    else if (number=0)
    {
        var db = app.db;
		db.transaction(function(tx) {
		tx.executeSql("UPDATE tblApp SET TOStatus='Checked', Status='Done', TOAStatus='Finished', AssetClientID=?, CompletionStamp=DATETIME() WHERE TransportOrderAssetID=?", [clientid,toaid],
					  app.onSuccess2, 
					  app.onError);
		}); 
    }
